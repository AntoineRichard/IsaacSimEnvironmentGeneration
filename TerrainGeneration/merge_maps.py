...
def assembleMap(folder, output_path, terrain_root = "/terrain"):
    ...
    for file in files:
        ...
        if extenstion.lower() != "usd":
            continue
        name = file.split('.')[0]
        y_coord = int(name.split('_')[0])
        x_coord = int(name.split('_')[1])
        if extenstion.lower() == "usd":
            file_path = os.path.join(folder, file)
            pu.createObject(os.path.join(terrain_root, "tile_"+name), stage, file_path, Gf.Vec3d(x_coord, y_coord, 0))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--maps", type=str, nargs="+", default=None, help="List of folders to convert (space seperated)."
    )
    parser.add_argument(
        "--name", type=str, default=None, help="The path where the maps will be stored."
    )
    ...
    processFolders(args.maps, args.name)
